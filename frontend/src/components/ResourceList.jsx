  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:3000/api/resource-details",
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          `An error occurred with network response: ${response.status}`
        );
      }
  ...
  return (
    <div className="flex mt-8 mb-8">
      <div className="flex-col mt-8 mb-8">
        <h1 className="text-center h-100 text-4xl font-bold justify-center text-blue-900">
          Find Your Resources
        </h1>
        <div className="mt-6 bg-gray-300 p-4 items-center justify-between flex rounded-md mx-auto max-w-2xl border-black border-2">
          <label htmlFor="filter" className="block font-bold text-black pl-2">
            Filter:
          </label>
          ...
          >
            ...
            <option value="consultation">Mental Health</option>
          </select>

          <button
            className="text-blue-900 font-bold pr-2"
